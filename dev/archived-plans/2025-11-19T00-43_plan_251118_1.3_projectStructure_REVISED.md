...
modules.forEach(({ path: modulePath, type, executable, name }) => {
  ...
  if (executable) {
    try {
      ...
      console.error(`✗ FAIL: ${modulePath} exists but is not executable (run: chmod +x ${modulePath})`);
      errors++;
    }
    return;
  }
...
try {
  ...
  if (!packageJson.bin || !packageJson.bin.transcriptor) {
    ...
    console.warn(`⚠ WARN: package.json bin.transcriptor points to ${packageJson.bin.transcriptor} instead of ./bin/transcriptor`);
    warnings++;
  } else {
...
function utilityFunction() { /* implementation */ }
