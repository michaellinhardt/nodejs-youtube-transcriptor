...
class APIClient {
  ...
  transformError(error) {
    ...
    switch (status) {
      ...
      case 429:
        return this.createAppError(APIClient.ERROR_TYPES.RATE_LIMITED, 'API rate limit exceeded', {
          status,
      ...
      default:
        return this.createAppError('UNKNOWN_HTTP_ERROR', `Unexpected HTTP status: ${status}`, {
          status,
          data: errorData,
        });
    }
  ...
  handleNetworkError(error) {
    ...
    if (this.isNetworkError(error)) {
      return this.createAppError(APIClient.ERROR_TYPES.NETWORK, `Network error: ${error.message}`, {
        code: error.code,
      });
    }
