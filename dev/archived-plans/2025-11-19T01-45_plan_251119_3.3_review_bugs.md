...
test('readTranscript throws correct error for empty file', async () => {
  ...
  await expect(
    storageService.readTranscript('test123')
  ).rejects.toThrow('Transcript file is empty for video ID: test123');
});
...
test('deleteTranscript succeeds when file already deleted', async () => {
  await expect(
    storageService.deleteTranscript('nonexistent')
...
test('deleteTranscript refuses to delete directories', async () => {
  ...
  ).rejects.toThrow('Cannot delete: path is not a file');
});
...
test('saveTranscript handles concurrent directory creation', async () => {
  ...
  for (let i = 0; i < 10; i++) {
    promises.push(
      storageService.saveTranscript(`test${i}abcde`, `content ${i}`)
    );
  }
...
test('saveTranscript distinguishes directory vs write errors', async () => {
  ...
  await expect(
    storageService.saveTranscript('test123', 'content')
  ).rejects.toThrow('Failed to write transcript test123: Disk full');
...
test('saveTranscript provides context for initialization failures', async () => {
  ...
  await expect(
    storageService.saveTranscript('test123', 'content')
  ).rejects.toThrow('Cannot save transcript (initialization failed): Permission denied');
...
test('readTranscript provides context for initialization failures', async () => {
  ...
  await expect(
    storageService.readTranscript('test123')
  ).rejects.toThrow('Cannot read transcript (initialization failed): EACCES');
...
test('deleteTranscript provides context for initialization failures', async () => {
  ...
  await expect(
    storageService.deleteTranscript('test123')
  ).rejects.toThrow('Cannot delete transcript (initialization failed): Disk full');
